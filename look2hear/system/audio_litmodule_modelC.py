...
class AudioLightningModule_ModelC(pl.LightningModule):
    def __init__(
        self,
        audio_model=None,
        video_model=None,
    ...
    def training_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            max_val = torch.max(torch.abs(preds), dim=1, keepdim=True)[0]
        preds_for_spk = preds / (max_val + 1e-8)
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx=0):
        ...
        # Validation speaker similarity monitoring
        with torch.no_grad():
            self.spk_encoder.to(self.device)
            val_est_embeddings = self.spk_encoder.encode_batch(est_sources_2d)
            val_est_dvec = val_est_embeddings.squeeze(1)
            val_cos_sim = F.cosine_similarity(val_est_dvec, dvec, dim=-1).mean()
        ...
        if dataloader_idx == 0:
            ...
            self.log("val/cos_sim", val_cos_sim, on_epoch=True, prog_bar=True, sync_dist=True)
            ...
            self.log("test/cos_sim", val_cos_sim, on_epoch=True, sync_dist=True)
