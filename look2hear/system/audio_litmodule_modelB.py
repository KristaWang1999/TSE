...
class AudioLightningModule_ModelB(pl.LightningModule):
    def __init__(
        self,
        audio_model=None,
        video_model=None,
        ...
        self.sample_rate = self.config["datamodule"]["data_config"]["sample_rate"]
    ...
    def training_step(self, batch, batch_nb):
            ...
            preds = est_sources.squeeze(1) # [B, T]
            
            loss_sisnr = self.loss_func["train"](preds, targets[:, 0, :])

            # --- 3. Speaker consistency loss (amplitude normalization) ---
            self.spk_encoder.to(self.device)
            
            # Peak normalization: prevents the model from shrinking amplitude to evade speaker penalty
            with torch.no_grad():
            ...
            est_dvec = est_embeddings.squeeze(1) # [B, 192]
            
            cos_sim = F.cosine_similarity(est_dvec, dvec, dim=-1)
            loss_spk = (1 - cos_sim).mean()

            # --- 4. Combined loss (alpha=30 to balance SI-SNR and speaker loss scales) ---
            alpha = 2
            ...
            self.log("train/raw_cos_sim", cos_sim.mean(), on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx=0):
            mixtures, targets, dvec = batch
                ...
                est_sources = self(mixtures, dvec)
            est_sources_2d = est_sources.squeeze(1) # [B, T]
            
            # 2. Current negative SI-SNR
            current_neg_sisnr = self.loss_func["val"](est_sources_2d, targets[:, 0, :])

            # 3. Baseline input SI-SNR
            with torch.no_grad():
                initial_neg_sisnr = self.loss_func["val"](mixtures.squeeze(1), targets[:, 0, :])
            
            # 4. SI-SNRi improvement
            si_snri = initial_neg_sisnr - current_neg_sisnr

            # Compute validation speaker similarity
            with torch.no_grad():
                self.spk_encoder.to(self.device)
                est_embeddings = self.spk_encoder.encode_batch(est_sources_2d)
                est_dvec = est_embeddings.squeeze(1) # [B, 192]
            ...
            if dataloader_idx == 0:
                ...
                test_total_loss = current_neg_sisnr + alpha * val_loss_spk
                self.log("test/loss", test_total_loss, on_epoch=True, sync_dist=True)
                self.log("test/si_snri", si_snri, on_epoch=True, prog_bar=True, sync_dist=True)
                self.log("test/spk_sim_loss", val_loss_spk, on_epoch=True, prog_bar=False, sync_dist=True)
