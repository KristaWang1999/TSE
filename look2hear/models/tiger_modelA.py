...
class TIGER(BaseModel):
    def __init__(
        ...
        self.audio_cos_proj = nn.Conv2d(out_channels, out_channels, 1)
        self.film_gen = nn.Linear(out_channels, out_channels * 2)
        

        self.post_injection = nn.Sequential(
    ...
    def forward(self, input, dvec):
        ...
        B_nch = dvec_emb.shape[0]

        audio_feat = subband_feature.permute(0, 2, 1, 3) # [B, 128, nband, T]
        audio_norm = F.normalize(self.audio_cos_proj(audio_feat), dim=1)
        dvec_norm = F.normalize(dvec_emb, dim=1).view(B_nch, 128, 1, 1)

        cos_sim = torch.sum(audio_norm * dvec_norm, dim=1, keepdim=True) # [B, 1, nband, T]
        cos_gate = torch.sigmoid(cos_sim * 10.0)  # temperature=10.0 sharpens the gate
        subband_feature = subband_feature * cos_gate.permute(0, 2, 1, 3)


        film_params = self.film_gen(dvec_emb).view(B_nch, 1, 256, 1) # [B, 1, 256, 1]
        gamma, beta = torch.chunk(film_params, 2, dim=2)  # each half = out_channels
        subband_feature = subband_feature * (1 + gamma) + beta

        subband_feature = self.post_injection(subband_feature.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)
