...
class TIGER(BaseModel):
    def __init__(
        ...
        self.dvec_proj = nn.Sequential(
            nn.Linear(dvec_dim, 256),
            nn.PReLU()
        )

        self.audio_cos_proj = nn.Conv2d(out_channels, 256, 1)
        self.film_gen = nn.Linear(256, out_channels * 2)
    ...
    def forward(self, input, dvec):
        ...
        dvec_norm = F.normalize(dvec_emb, dim=1).view(B_nch, 256, 1, 1)
