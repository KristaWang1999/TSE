...
class UConvBlock(nn.Module):
    ...
    def __init__(self, out_channels=128, in_channels=512, upsampling_depth=4, model_T=True):
        ...
        self.film_gamma = nn.Linear(out_channels, in_channels)  # out_channels = projected dvec dim
        self.film_beta = nn.Linear(out_channels, in_channels)

        # Weight initialization
        nn.init.zeros_(self.film_gamma.weight)
        nn.init.ones_(self.film_gamma.bias)   # gamma initialized to 1
        nn.init.zeros_(self.film_beta.weight)
        nn.init.zeros_(self.film_beta.bias)   # beta initialized to 0
    ...
    def forward(self, x, dvec_emb):
        ...
        gamma = self.film_gamma(dvec_emb).unsqueeze(-1)  # (B_new, in_channels, 1)
        beta = self.film_beta(dvec_emb).unsqueeze(-1)
        output1 = output1 * gamma + beta

        
        output = [self.spp_dw[0](output1)]
        for i in range(1, self.depth):
            output.append(self.spp_dw[i](output[i-1]))
...
class MultiHeadSelfAttention2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dvec_emb=None):
        if self.dim == 4:
...
class Recurrent(nn.Module):
    ...
    def forward(self, x, dvec_emb):
        ...
        for i in range(self.iter):
            if i == 0:
                x = self.freq_time_process(x, B, nband, N, T, dvec_emb) # B, N, nband, T
            else:
                x = self.freq_time_process(self.concat_block(mixture + x), B, nband, N, T, dvec_emb)
                
        return x.permute(0, 2, 1, 3).contiguous() # B, nband, N, T
    
    def freq_time_process(self, x, B, nband, N, T, dvec_emb):
        # Process Frequency Path
        residual_1 = x.clone()
        x = x.permute(0, 3, 1, 2).contiguous() # B, T, N, nband

        dvec_freq = dvec_emb.unsqueeze(1).repeat(1, T, 1).view(B*T, -1)

        freq_fea = self.freq_path[0](x.view(B*T, N, nband), dvec_freq)
        freq_fea = freq_fea.view(B, T, N, nband).permute(0, 2, 1, 3).contiguous() # B, N, T, nband

        freq_fea = self.freq_path[1](freq_fea, dvec_emb)
        ...
        dvec_frame = dvec_emb.unsqueeze(1).repeat(1, nband, 1).view(B*nband, -1)

        frame_fea = self.frame_path[0](x2.view(B*nband, N, T), dvec_frame)
        frame_fea = frame_fea.view(B, nband, N, T).permute(0, 2, 1, 3).contiguous() # [B, N, nband, T]
        frame_fea = self.frame_path[1](frame_fea, dvec_emb)
        frame_fea = self.frame_path[2](frame_fea)
        x = frame_fea + residual_2
        return x
        
class TIGER_ModelB(BaseModel):
    def __init__(
        self,
        out_channels=128,
        in_channels=512,
        ...
        super(TIGER_ModelB, self).__init__(sample_rate=sample_rate)
        ...
        # Speaker embedding projection layer
        self.dvec_proj = nn.Sequential(
            nn.Linear(dvec_dim, out_channels),
            nn.PReLU()
        )
        ...
        self.bottleneck_cos_proj = nn.Conv2d(out_channels, out_channels, 1)

        # FiLM modulation parameter generator for bottleneck injection
        self.bottleneck_film = nn.Linear(out_channels, out_channels * 2)

        # Post-injection normalization for stable feature distribution
        self.bottleneck_norm = nn.GroupNorm(1, out_channels)
        self.bottleneck_prelu = nn.PReLU()

        nn.init.zeros_(self.bottleneck_film.weight)
        nn.init.constant_(self.bottleneck_film.bias, 0.0)
    ...
    def forward(self, input, dvec):
        ...
        #--- Model B: FiLM Condition Preparation ---
        # 1. Project speaker embedding
        dvec_emb = self.dvec_proj(dvec)

        # 2. Repeat dvec for multi-channel inputs
        if nch > 1:
            dvec_emb = dvec_emb.repeat_interleave(nch, dim=0)

        feat_for_bottleneck = subband_feature.permute(0, 2, 1, 3).contiguous()
        # 1. Cosine gating for coarse speaker filtering
        norm_audio = F.normalize(self.bottleneck_cos_proj(feat_for_bottleneck), p=2, dim=1)
        norm_dvec = F.normalize(dvec_emb, p=2, dim=1).view(batch_size * nch, self.feature_dim, 1, 1)
        cos_sim = torch.sum(norm_audio * norm_dvec, dim=1, keepdim=True) # [B*nch, 1, nband, T]
        cos_gate = torch.sigmoid(cos_sim * 10.0)  # temperature=10.0 for sharper contrast
        feat_for_bottleneck = feat_for_bottleneck * cos_gate

        # 2. FiLM affine modulation for speaker guidance
        b_film_params = self.bottleneck_film(dvec_emb).view(batch_size * nch, self.feature_dim * 2, 1, 1)
        b_gamma, b_beta = torch.chunk(b_film_params, 2, dim=1)
        feat_for_bottleneck = feat_for_bottleneck * (1 + b_gamma) + b_beta

        # 3. Normalize and restore shape to [B*nch, nband, N, T]
        feat_for_bottleneck = self.bottleneck_norm(feat_for_bottleneck)
        feat_for_bottleneck = self.bottleneck_prelu(feat_for_bottleneck)
        subband_feature = feat_for_bottleneck.permute(0, 2, 1, 3).contiguous()

        # --- Level 2: Internal FFI Injection ---
        sep_output = self.separator(
            subband_feature,  # Level 1 filtered features
            dvec_emb
        )   
        
        
        
        norm_sep = F.normalize(sep_output, p=2, dim=2) 
        # 2. Normalize dvec: (B, 1, C, 1)
        dvec_for_gate = dvec_emb.view(batch_size * nch, 1, self.feature_dim, 1)
        norm_dvec = F.normalize(dvec_for_gate, p=2, dim=2)
        
        # 3. Cosine similarity with scale factor for sharper gate
        match_score = torch.sum(norm_sep * norm_dvec, dim=2, keepdim=True)
        gate = torch.sigmoid(match_score * 10.0) 
        
        sep_output = sep_output * gate
