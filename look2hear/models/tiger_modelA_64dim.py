...
class TIGER(BaseModel):
    def __init__(
        ...
        self.dvec_proj = nn.Sequential(
            nn.Linear(dvec_dim, 64),
            nn.PReLU()
        )

        self.audio_cos_proj = nn.Conv2d(out_channels, 64, 1)
        self.film_gen = nn.Linear(64, out_channels * 2)
    ...
    def forward(self, input, dvec):
        ...
        dvec_norm = F.normalize(dvec_emb, dim=1).view(B_nch, 64, 1, 1)
