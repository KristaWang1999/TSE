...
def main(config, model_path=None, pretrain_path=None):
    ...
    # This way, even if __init__.py wasn't changed, this will precisely load the class from tiger_modelC.py
    from look2hear.models.tiger_modelC import TIGER_ModelC
    from look2hear.datas.Libri2Mix16_modelC import Libri2MixModule_ModelC as DataModule
    ...
    config["main_args"]["exp_dir"] = os.path.join(
        os.getcwd(), "Experiments_modelC", "checkpoint", config["exp"]["exp_name"]
    )
    ...
    print_only("Instantiating System <Model C FiLM Version>")
    ...
    logger_dir = os.path.join(os.getcwd(), "Experiments_modelC", "tensorboard_logs")
